...
<div class="col-md-3">
    ...
    <div><?= lang("Validation.companiesunder"); ?> <?= $this->session->userdata('project_name'); ?></div><br>
    <?php foreach ($com_pro as $cp): ?>
        <div class="boxhead"><?= $cp['company_name']; ?></div>
        <div class="boxcontent">
            <a href="<?= base_url('cpscoping/'.$this->session->userdata('project_id').'/'.$cp['company_id'].'/allocation'); ?>/" class="btn btn-inverse btn-sm" id="cpscopinga"><?= lang("Validation.createallocation"); ?></a>
