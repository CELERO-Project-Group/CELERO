...
<div class="col-md-3">
    ...
    <div><?= lang("companiesunder"); ?> <?= $this->session->userdata('project_name'); ?></div><br>
    <?php foreach ($com_pro as $cp): ?>
        <div class="boxhead"><?= $cp['company_name']; ?></div>
        <div class="boxcontent">
            <a href="<?= base_url('cpscoping/'.$this->session->userdata('project_id').'/'.$cp['company_id'].'/allocation'); ?>/" class="btn btn-inverse btn-sm" id="cpscopinga"><?= lang("createallocation"); ?></a>
            <a href="<?= base_url('new_flow/'.$cp['company_id']); ?>/" class="btn btn-inverse btn-sm" id="cpscopinga"><?= lang("datasetmanagement"); ?></a>
        </div>
    <?php endforeach ?><br>
</div>
...
<!--<div class="col-md-6" style="border-left: 1px solid #C3C3C3;"><b>Project</b><br><a href="<?= base_url('project/'.$cp['project_id']); ?>"><?= $cp['project_name']; ?></a></div> -->
        ...
        <div class="cp-bar">
            <a style="margin-right:10px;" href="<?= base_url('cpscoping/'.$cp['project_id'].'/'.$cp['company_id'].'/show'); ?>" class=" btn-sm btn-info"><?= lang("viewcp"); ?></a>
            <a style="margin-right:10px;" href="<?= base_url('kpi_calculation/'.$cp['project_id'].'/'.$cp['company_id']); ?>" class=" btn-sm btn-success"><?= lang("viewkpi"); ?></a>
            <a href="<?= base_url('cost_benefit/'.$cp['project_id'].'/'.$cp['company_id']); ?>" class=" btn-sm btn-warning"><?= lang("viewcba"); ?></a>
        </div>
        <table class="table table-striped" style="font-size:12px;">
            <tr>
                <th style="width: 500px;"><?= lang("processname"); ?></th>
                <th style="width: 400px;"><?= lang("flowname"); ?></th>
                <th style="width: 200px;"><?= lang("flowtype"); ?></th>
                <th><?= lang("manage"); ?></th>
            </tr>
        <?php endif ?>
        <?php for($k = 0 ; $k < sizeof($flow_prcss[$i]) ; $k++): ?>
            ...
            <tr>
                ...
                <td>
                    <a class="label label-info" href="<?= base_url('cpscoping/edit_allocation/'.$flow_prcss[$i][$k]['allocation_id']); ?>"><?= lang("editallocation"); ?></a>
                    <a class="label label-danger" href="<?= base_url('cpscoping/delete/'.$flow_prcss[$i][$k]['allocation_id'].'/'.$flow_prcss[$i][$k]['project_id'].'/'.$flow_prcss[$i][$k]['company_id']); ?>" 
                        onclick="return confirm('Are you sure you want to delete the allocation <?= $flow_prcss[$i][$k]['prcss_name']; ?>?');"><?= lang("deleteallocation"); ?></a></td>
            </tr>   
        <?php endfor ?>
        </table>
        <?php $i++; ?>
