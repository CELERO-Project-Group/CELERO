	...
	<div class="row">
			...
			<div class="form-group">
				...
				<?php foreach ($cmpnyperson as $cmpprsn): ?>
					...
					<a href="<?= base_url("removeUserfromCompany/".$companies['id']."/".$cmpprsn['id']) ?>"><i class="fa fa-pencil-square-o"></i> remove</a>
				<?php endforeach ?>
				</ul>
			</div>
			<?php if($canDelete=='1'): ?>
		...
		<div class="col-md-8">
			<div class="swissheader"><?= $companies['name'] ?></div>
			...
			<?php if($valid != 0): ?>
				...
				<table class="table table-bordered">
					...
					<?php foreach ($company_flows as $flows): ?>
						<tr>
							...
							<td><?= $flows['flowtype']; ?></td>
							<td class="table-numbers"><?= number_format($flows['qntty'], 0, ".", "'") ?></td>
							<td class="table-units"><?= $flows['qntty_unit_name'] ?></td>
							<td class="table-numbers"><?= number_format($flows['cost'], 0, ".", "'") ?></td>
							<td class="table-units"><?= $flows['cost_unit'] ?></td>
							<td style="text-align: right"><?= number_format($flows['ep'], 0, ".", "'") ?></td>
						</tr>
					<?php endforeach ?>
				</table>
				...
				<table class="table table-bordered">
					...
					<?php foreach ($company_prcss as $prcss): ?>
						<tr>
							<td><?= $prcss['prcessname'] ?></td>
							<td><?= $prcss['flowname'] ?></td>
							<td><?= $prcss['flow_type_name'] ?></td>
						</tr>
					<?php endforeach ?>
				</table>
				...
				<table class="table table-bordered">
					...
					<?php foreach ($company_component as $cmpnnt): ?>
						<tr>
							<td><?= $cmpnnt['flow_name'] ?></td>
							<td><?= $cmpnnt['component_name'] ?></td>
						</tr>
					<?php endforeach ?>
				</table>
